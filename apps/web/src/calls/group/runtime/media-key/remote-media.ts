/**
 * remote-media — remote participant media snapshot types and builder.
 *
 * Owns:
 *   - RemoteVideoSlot — single video producer slot (stream, source, deviceId)
 *   - RemoteParticipantMediaInternal — mutable internal per-user media state used
 *     inside the consumer runtime (Maps keyed by producerId)
 *   - GroupCallRemoteMedia — immutable public snapshot consumed by the UI layer
 *   - buildRemoteMediaSnapshot — converts the internal Map to a sorted GroupCallRemoteMedia[]
 *     with audio attached to the first video slot per user and source-priority ordering
 *     (camera < screen < other)
 *
 * Does not own consumer lifecycle or frame-crypto handle management
 * (see consumer-runtime.ts and runtime-common.ts).
 */
