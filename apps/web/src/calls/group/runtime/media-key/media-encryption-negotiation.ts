/**
 * media-encryption-negotiation — group call E2EE mode resolution and negotiation helpers.
 *
 * Owns:
 *   - GroupCallRuntimeMediaEncryptionMode type alias (mirrors the protocol type)
 *   - LocalGroupCallMediaEncryptionDecision — the full local decision record
 *   - resolveLocalGroupCallMediaEncryptionDecision — maps user's CallSecurityMode setting
 *     to requested/advertised mode considering browser frame-transform capability
 *   - resolveMostCompatibleGroupMediaEncryptionMode — picks the lowest common mode from
 *     local preference and all remote participant modes
 *   - resolveEffectiveGroupMediaEncryptionMode — extends the above with an option to
 *     treat unknown-remote modes as "off" (treatUnknownRemoteAsOff)
 *   - isGroupMediaModeDowngraded — predicate for UI downgrade warnings
 *   - shouldArmLocalGroupCallFrameEncryption — decides whether to arm frame encryption
 *     based on mode, expected devices, and ACK count
 *   - supportsInsertableStreams — re-export of supportsEncodedFrameTransforms
 *
 * Does not own key generation, delivery, or SFU transport selection.
 *
 * Note: frame-level E2EE is reserved for strict mode; balanced mode currently
 * stays on transport encryption per the comment in resolveLocalGroupCallMediaEncryptionDecision.
 */
