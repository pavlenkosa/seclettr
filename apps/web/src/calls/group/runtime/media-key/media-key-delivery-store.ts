/**
 * media-key-delivery-store — sessionStorage-backed delivery persistence for media keys.
 *
 * Owns:
 *   - createSessionStorageDeliveryStore — returns a GroupCallMediaKeyDeliveryStore that
 *     persists delivered delivery keys (targetDeviceId:keyId) across WS reconnects via
 *     sessionStorage; capped at MAX_ENTRIES (500) entries to prevent unbounded growth
 *
 * Does not own the delivery tracker or retry logic (see media-key-delivery.ts).
 * Storage key format: "sc.mkd.{callId}:{deviceId}".
 */
