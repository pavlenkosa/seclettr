...
describe("plain-groups-store", () => {
  ...
  it("optimistically patches edited content after a successful edit", async () => {
    await seedGroup();
    wsHandler?.({
      type: "plain_message.new",
      message: {
        id: "srv-edit-1",
        clientId: "wire-edit-1",
        senderUserId: "bob",
        senderUsername: "Bob",
        groupId: "g1",
        content: "before",
    ...
    apiPatchMock.mockResolvedValueOnce(undefined);
    await usePlainGroupsStore.getState().editMessage("g1", "srv-edit-1", "after");

    const msg = groups()["g1"]?.messages.find((m) => m.id === "srv-edit-1");
    expect(msg?.content).toBe("after");
    expect(typeof msg?.editedAt).toBe("number");
  });

  it("removes a message locally after a successful delete", async () => {
    await seedGroup();
    wsHandler?.({
      type: "plain_message.new",
      message: {
    ...
    apiDeleteMock.mockResolvedValueOnce(undefined);
    await usePlainGroupsStore.getState().deleteMessage("g1", "srv-del-1");

    expect(groups()["g1"]?.messages.some((m) => m.id === "srv-del-1")).toBe(false);
  });
  ...
  it("applies a WS edit event to an existing group message", async () => {
    await seedGroup();
    wsHandler?.({
      type: "plain_message.new",
      message: {
        id: "srv-ws-edit-1",
        clientId: "wire-ws-edit-1",
        senderUserId: "bob",
        senderUsername: "Bob",
        groupId: "g1",
        content: "before ws edit",
        messageType: "text",
        createdAt: new Date(1_700_000_000_000).toISOString(),
      },
    });
    ...
    wsHandler?.({
      ...
      messageId: "srv-ws-edit-1",
      content: "after ws edit",
      editedAt: new Date(1_700_000_010_000).toISOString(),
      threadKey: "g1",
      threadKind: "group",
    });

    const msg = groups()["g1"]?.messages.find((m) => m.id === "srv-ws-edit-1");
    expect(msg?.content).toBe("after ws edit");
    expect(typeof msg?.editedAt).toBe("number");
  });

  it("applies a WS delete event to an existing group message", async () => {
    await seedGroup();
    wsHandler?.({
      type: "plain_message.new",
      message: {
        id: "srv-ws-del-1",
        clientId: "wire-ws-del-1",
        senderUserId: "bob",
        senderUsername: "Bob",
        groupId: "g1",
        content: "delete via ws",
        messageType: "text",
        createdAt: new Date(1_700_000_000_000).toISOString(),
      },
    });
    ...
    wsHandler?.({
      ...
      messageId: "srv-ws-del-1",
      threadKey: "g1",
      threadKind: "group",
    });

    expect(groups()["g1"]?.messages.some((m) => m.id === "srv-ws-del-1")).toBe(false);
  });
  ...
  it("clears all groups on reset and preserves current wsConnected snapshot", async () => {
    await seedGroup();
    expect(Object.keys(groups())).toContain("g1");

    usePlainGroupsStore.getState().reset();

    expect(groups()).toEqual({});
    expect(usePlainGroupsStore.getState().wsConnected).toBe(true);
  });
});
