/**
 * media-key-delivery — reliable media-key delivery tracker with retry and ACK.
 *
 * Owns:
 *   - GroupCallMediaKeyDeliveryTracker interface (share, acknowledge, isDelivered, clear)
 *   - GroupCallMediaKeyDeliveryStore interface for persistent delivery state
 *   - createGroupCallMediaKeyDeliveryTracker factory:
 *       - Per-target pending delivery tracking keyed by targetDeviceId:keyId
 *       - Retry scheduling at configurable delays (default: 1.5 s / 4 s / 9 s)
 *       - onDeliveryExhausted callback after all retry attempts are consumed
 *       - acknowledge() validates the ACK signal and marks the delivery as complete
 *       - Persistent delivery set via GroupCallMediaKeyDeliveryStore (survives WS reconnect)
 *
 * Does not own ACK proof computation (see media-key-ack-proof.ts), the WS send
 * primitive, or key encryption. The tracker is WS-send-agnostic: callers inject
 * the send function.
 */
